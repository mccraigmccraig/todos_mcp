defmodule TodosMcp.CommandProcessorTest do
  ...
  describe "build/1 with AsyncRunner" do
    ...
    test "executes a command and yields result" do
      ...
      {:yield, {:ok, todo}} = AsyncRunner.resume_sync(runner, %CreateTodo{title: "Test todo"})
    ...
    test "processes multiple commands in sequence" do
      ...
      # Create first todo
      {:yield, {:ok, todo1}} = AsyncRunner.resume_sync(runner, %CreateTodo{title: "First"})
      assert todo1.title == "First"

      # Create second todo
      {:yield, {:ok, todo2}} = AsyncRunner.resume_sync(runner, %CreateTodo{title: "Second"})
      assert todo2.title == "Second"

      # List todos
      {:yield, {:ok, todos}} = AsyncRunner.resume_sync(runner, %ListTodos{})
    ...
    test "toggle and delete commands" do
      ...
      # Create
      {:yield, {:ok, todo}} = AsyncRunner.resume_sync(runner, %CreateTodo{title: "Toggle me"})
      assert todo.completed == false

      # Toggle
      {:yield, {:ok, toggled}} = AsyncRunner.resume_sync(runner, %ToggleTodo{id: todo.id})
      assert toggled.completed == true

      # Delete
      {:yield, {:ok, deleted}} = AsyncRunner.resume_sync(runner, %DeleteTodo{id: todo.id})
    ...
    test "respects tenant isolation" do
      ...
      {:ok, runner1, {:yield, :ready}} = AsyncRunner.start_sync(processor1, tag: :cmd1)
      {:ok, runner2, {:yield, :ready}} = AsyncRunner.start_sync(processor2, tag: :cmd2)

      # Create in tenant 1
      {:yield, {:ok, _}} = AsyncRunner.resume_sync(runner1, %CreateTodo{title: "Tenant 1 todo"})

      # Create in tenant 2
      {:yield, {:ok, _}} = AsyncRunner.resume_sync(runner2, %CreateTodo{title: "Tenant 2 todo"})

      # List in tenant 1 - should only see tenant 1's todo
      {:yield, {:ok, todos1}} = AsyncRunner.resume_sync(runner1, %ListTodos{})
      assert length(todos1) == 1
      assert hd(todos1).title == "Tenant 1 todo"

      # List in tenant 2 - should only see tenant 2's todo
      {:yield, {:ok, todos2}} = AsyncRunner.resume_sync(runner2, %ListTodos{})
    ...
    test "handles errors gracefully" do
      ...
      {:yield, {:ok, todo}} = AsyncRunner.resume_sync(runner, %CreateTodo{title: "After error"})
  ...
  describe "async usage pattern" do
    test "can use async resume for commands" do
      ...
      # Receive the result via message
      assert_receive {:cmd, :yield, {:ok, todo}}
